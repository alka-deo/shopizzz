...
def generate_synthetic_data(n_samples=1000):
    ...
    base_wtp = (
        data['monthly_spending'].apply(lambda x: ['<1K', '1-3K', '3-5K', '5-10K', '>10K'].index(x)) * 0.3 +
    ...
    if st.button("🔍 Mine Association Rules", type="primary"):
        with st.spinner("Mining association rules..."):
            ...
            if len(rules) <= 20:
                ...
                st.info("Network graph not shown for >20 rules. Displaying top 20.")
    ...
    if st.button("🚀 Train Models", type="primary"):
        with st.spinner("Training regression models..."):
            ...
            fig.add_trace(go.Bar(name='Adjusted R²', x=list(results.keys()),
                               y=[r['adjusted_r2'] for r in results.values()],
...
def dynamic_pricing_tab(df):
    ...
    with col1:
        ...
        location = st.selectbox("Location", ['Metro', 'Tier-1', 'Tier-2', 'Tier-3'])
    
    with col2:
        income = st.selectbox("Income Level", ['<25K', '25-50K', '50-75K', '75-100K', '100-150K', '>150K'])
        monthly_spending = st.selectbox("Current Monthly Spending", 
                                       ['<1K', '1-3K', '3-5K', '5-10K', '>10K'])
        shopping_style = st.selectbox("Shopping Style", 
                                     ['Budget', 'Balanced', 'Premium', 'Luxury'])
    
    with col3:
        sustainability = st.selectbox("Sustainability Consciousness", 
                                     ['Low', 'Medium', 'High', 'Very High'])
        artisan_support = st.selectbox("Artisan Support Interest", 
                                      ['Low', 'Medium', 'High', 'Very High'])
    ...
    if st.button("💡 Calculate Optimal Price", type="primary"):
        ...
        # Convert WTP to price range
        # Mapping: 0-7 scale to actual price ranges
        wtp_mapping = {
...
def main():
    ...
    if 'df' not in st.session_state or st.session_state['df'] is None:
        st.warning("⚠️ Please load data from the sidebar to begin analysis")
